...
%
% This function implements the RKFIT algorithm [1, 2] used for
% nonlinear rational least squares fitting.
%
% In the simple case when F and A are N-by-N matrices and b is an
...
% be an N-by-n block of vectors. Additional weighting matrices can
% be passed, as well as additional parameters detailed below.
%
...
%             perform matrix-matrix multiplication, e.g.,
%             F = @(X) Fmat*X; alternatively, F may be a cell array
%             containing ell such objects;
%   - A,      N-by-N matrix, or a struct representing an N-by-N
%             pencil, see the RAT_KRYLOV function for details;
%   - b,      N-by-n matrix;
...
%                  of the type (m+k, m) of the rational
%                  approximants;
...
%                  variable is set to 1, provided that a relative
%                  misfit below param.tol has been reached;
% param.return,    'best' (default) or 'last'; which of the rkfuns
%                  to return as judged by the misfit; if reductions
%                  took place and 'best' is on, it will return the
%                  best approximant with lower denominator degree
%                  only if it provides a misfit below param.tol,
%                  otherwise the best approximant with higher degree;
% param.safe,      safety parameter for the reduction, defaults to
%                  0.1, see (4.3) in [2] for details;
...
%                  by default 1e-15.
%
% Other allowed fields in param are those used by RAT_KRYLOV.
%
...
%       misfit_initial: relative misfit of the starting guess;
...
%         reduction_it: NaN or iteration number when reduction occurred;
%            return_it: iteration of which approximant is returned;
...
%     Real-valued Hessenberg pencil (H, K) of size 2-by-1.
%     coeffs = [ 0.676, 0.258 ]
%
% References:
%
