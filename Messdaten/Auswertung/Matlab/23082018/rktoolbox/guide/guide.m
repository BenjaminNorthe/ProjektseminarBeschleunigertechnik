...
% current version 2.7 provides
%
% * an implementation of Ruhe's (block) rational Krylov sequence method [8, 9], 
% allowing to control various options, including user-defined 
% inner products, exploitation of complex-conjugate shifts,
% orthogonalization, rerunning [3], and parallelism [4], 
...
% and the Advanpix Multiple Precision toolbox [1], and
...
% the NLEIGS [6] and AAA algorithms [7].
%
...
% rational Krylov method by Ruhe [5, 6] computes an orthonormal
...
% of $A$'s eigenvalues [8, 9]. Other applications include
...
%%
% With the help of the AAA algorithm [7] we sample this matrix at
% sufficiently many points in the search interval and construct an accurate
% rational interpolant, which is then converted into RKFUNM format; see [7]
...
xi = zeros(1,25); % shifts for the Krylov space
[m,n] = type(ratm);
...
ritzval = diag(D); % Ritz values
ritzval = ritzval(abs(imag(ritzval))< 1e-7 & abs(ritzval)<pi);
...
% RKT_BIGBREAK
% 
% [6] S. Guettel, R. Van Beeumen, K. Meerbergen, and W. Michiels. 
...
% RKT_BIGBREAK
% 
% [7] Y. Nakatsukasa, O. Sete, and L. N. Trefethen. 
%     _The AAA algorithm for rational approximation,_
%     arXiv preprint arXiv:1612.00337, 2016.
%
% RKT_BIGBREAK
%
% [8] A. Ruhe. _Rational Krylov: A practical algorithm for large sparse
%     nonsymmetric matrix pencils,_ SIAM J. Sci. Comput., 19(5):1535--1551,
%     1998.
%
% RKT_BIGBREAK
%
% [9] A. Ruhe. _The rational Krylov algorithm for nonsymmetric eigenvalue
