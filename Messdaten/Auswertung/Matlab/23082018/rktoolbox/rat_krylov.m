...
%   (I)   run the (block) rational Arnoldi algorithm [4, 5];
...
%   - V,      N-by-ns matrix;
%   - xi,     1-by-m row-vector, with (m + n)s < N;
%   - K, H,   upper-Hessenberg matrices with one block-row more than columns;
...
%                            Currently only available for s = 1.
%
...
if rerun
    ...
    run_krylov; % Both running and extending use the same function.
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function run_krylov
        bd = false;
        bd_tol = param.deflation_tol; % Can separate two tols if needed.
        ...
        if j == 1 % If creating a new RAD.
            ...
            [V(:,1:s), R(1:s,1:s)] = util_qr(V(:,1:s), param.inner_product);
            r_b(2) = s+1;
            param.last_full = 1;
        else
            r_b = param.r_b;
            ...
            if isreal(xi(j-offset)) || realopt == 0
                switch param.orth
                    case 'CGS'
                        ...
                        perm = E;
                        E(E) = 1:length(E); % inverse permutation
                        RR = RR(:,E); % permute RR
                        % 'CGS' end
                ...
                if norm(RR,'fro') ...
                        <= bd_tol*norm(H(1:r_b(j+1)-1, c_b(j):c_b(j)+s-1),'fro')
                    bd = true;
                    r_b(j+2) = r_b(j+1); c_b(j+1) = c_b(j);
                    H(1:r_b(j+2)-1, c_b(j):c_b(j)+s-1) = zeros(r_b(j+2)-1, s);
                    j = j+1;
                    continue;
                end
                
                diagr = svd(RR);
                r = find(diagr > diagr(1)*param.deflation_tol, 1, 'last'); % Rank estimation.
                if isempty(r), r = 0; end
                if r == s
                    param.last_full = j+1; % No deflation occurred so use for next continuation vector.
                end
                
                r_b(j+2) = r_b(j+1) + r; c_b(j+1) = c_b(j) + s;
                H(r_b(j+1):r_b(j+2)-1,c_b(j):c_b(j)+s-1) = RR(1:r,:); % Add subdiag to H.
                V(:,r_b(j+1):r_b(j+2)-1) = w(:,1:r); % Add new vectors to V.
                
                col = s;
                inds = sort(c_b(j)-1 + perm(1:r));
                ...
                K(1:r_b(j+2)-1, c_b(j):c_b(j+1)-1) = nu(j-offset)*K(1:r_b(j+2)-1, c_b(j):c_b(j+1)-1) - ...
                    rho(j-offset)*[T(1:r_b(j+1)-1, c_b(j):c_b(j+1)-1); zeros(r,col)];
                % Construct H.
                H(1:r_b(j+2)-1, c_b(j):c_b(j+1)-1) = mu(j-offset)*H(1:r_b(j+2)-1, c_b(j):c_b(j+1)-1) - ...
                    eta(j-offset)*[T(1:r_b(j+1)-1, c_b(j):c_b(j+1)-1); zeros(r,col)];
                ...
                r_b(j+2) = r_b(j+1) + r; c_b(j+1) = c_b(j) + s;
                H(r_b(j+1):r_b(j+2)-1, c_b(j):c_b(j)+s-1) = RR(1:r,:); % Add subdiag to H.
                V(:,r_b(j+1):r_b(j+2)-1) = real_w(:,1:r); % Add new vectors to V.
                inds = c_b(j)-1 + (1:s);
                param.column_deflation(inds) = ones(1,length(inds));
                j = j+1;
                ...
                r_b(j+2) = r_b(j+1) + r; c_b(j+1) = c_b(j) + s;
                H(r_b(j+1):r_b(j+2)-1,c_b(j):c_b(j)+s-1) = RR(1:r,:); % Add subdiag to H.
                V(:,r_b(j+1):r_b(j+2)-1) = imag_w(:,1:r); % Add new vectors to V.
                inds = c_b(j)-1 + (1:s);
                param.column_deflation(inds) = ones(1,length(inds));
        ...
        if bd == true
            warning(['rat_krylov: ''lucky breakdown'' occured, see ' ...
                'blocksize.']);
            % Break occurred before index j was increased.
        end
        
        % When extendng, the previous block sizes are unknown. This replaces the
        % previous block size with NaN.
        ...
        % Remove zero rows and columns in event of deflation or lucky
        % breakdowns
        V(:,r_b(j+1):size(V,2)) = [];
        ...
        function continuation_pair
            ...
            switch param.continuation
                case 'almost-last'
                    assert(s == 1 , 'almost_last unavailable for block case')
                    j_loc = max(1, j-param.p+1);
                    T(1:r_b(j+1)-1, c_b(j):c_b(j)+s-1) = zeros(r_b(j+1)-1, c_b(j)+s-c_b(j));
                    T(r_b(j_loc):r_b(j_loc+1)-1, c_b(j):c_b(j)+s-1) = eye(r_b(j+1)-r_b(j));
                ...
                case 'last'
                    ...
                    T(1:r_b(j+1)-1, c_b(j):c_b(j)+s-1) = zeros(r_b(j+1)-1, s);
                ...
                case 'near-optimal'
                    ...
                    assert(s == 1 , 'near-optimal currently unavailable for block case')
                    if j_loc == 1
                        T(j_loc, j) = 1; 
                    else
    ...
    function rerun_krylov
        ...
        while j <= m
            ...
            elseif realopt == 1 || realopt == 2
                ...
                % This select must correspond to the ordering of the poles
                % chosen in the function util_pencil_poles.
...
switch rat_krylov_mode
    ...
    case {3} % Extending.
        ...
        assert(size(varargin{V_id+1}, 1) == j, ...
            [msg 'K has to have j rows'])
        assert(size(varargin{V_id+2}, 1) == j, ...
            [msg 'H has to have j rows'])
...
if ~isfield(param, 'p') % Set to default 1.
    param.p = 1;
else % If stated, set to 1 for block case
    if s ~= 1 && param.p ~= 1
        warning('Warning: parallel execution not supported for block case. Setting to default')
        param.p = 1;
    end
end
